...
def is_abbr_of(a, b, partial=False, loose=False):
    ...
    common_words = set(abbr_part_a + full_part_a).intersection(
        set(abbr_part_b + full_part_b))
    ...
    if loose:
        first_chars_a, first_chars_b = get_first_chars(
            ' '.join(abbr_part_a + full_part_a)), get_first_chars(
                ' '.join(abbr_part_b + full_part_b))
        if partial and (len(first_chars_a) == 0 or len(first_chars_b) == 0):
            return True
        if not (set(first_chars_b.keys()).issubset(first_chars_a.keys())
                or set(first_chars_a.keys()).issubset(first_chars_b.keys())):
            return False
        ...
        check_flag = False
        for aA in abbr_part_a:
            if b.find(aA) == -1:
                check_flag = True
                break
        if check_flag:
            for bB in abbr_part_b:
                if a.find(bB) == -1:
                    return False

        return True

    abbr_part_b_copy, full_part_b_copy = copy.copy(abbr_part_b), copy.copy(
        full_part_b)
...
def match_name_one(main_name, dupl_name, loose=False):
    if same_name(main_name, dupl_name):
        return True
    part_a, full_a = split_abbr_full(main_name)
    part_b, full_b = split_abbr_full(dupl_name)
...
def match_name_two(main_name, dupl_name, loose=False):
    main_is_chinese, dupl_is_chinese = is_chinese_name(
        main_name), is_chinese_name(dupl_name)
    full_a, full_b = [], []
    if main_is_chinese and dupl_is_chinese:
        ...
        if len(abbr_a) == 0 and len(abbr_b) == 0:
            return False
    if (main_is_chinese and len(main_name.split()) < 2) or (
            dupl_is_chinese and len(dupl_name.split()) < 2):
        return False
    if loose:
        if (main_is_chinese or dupl_is_chinese) and (len(full_a) == 0
                                                     or len(full_b) == 0):
            return False

        return is_abbr_of(main_name, dupl_name, loose=True)
    return is_abbr_of(main_name, dupl_name, loose=False) and get_first_chars(
        main_name) == get_first_chars(dupl_name)
...
def match_name_five(main_name, dupl_name, loose=False):
    ...
    return main_words[-1] == dupl_words[-1][:-1] or dupl_words[
        -1] == main_words[-1][:-1]
...
def match_name_six(main_name, dupl_name, loose=False):
    ...
    main_name, dupl_name = main_name.replace(" ",
                                             ""), dupl_name.replace(" ", "")
    if main_name != dupl_name and (main_name.startswith(dupl_name)
                                   or dupl_name.startswith(main_name) and
                                   abs(len(main_name) - len(dupl_name)) < 3):
        return True
    return False
